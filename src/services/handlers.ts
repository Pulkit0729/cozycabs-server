...
export async function bookFromBlabla(
  user_phone: string,
  user_name: string,
  seats: any,
  ...
  try {
    ...
    let ride = await Ride.findOne({ blabla_ride_id: blabla_ride_id });
...
export async function book(
  ...
  try {
    ...
    let ride = await Ride.findOne({ ride_no: Number(ride_no) });
...
export async function cancel(
  ...
  try {
    ...
    if (ride_id) {
      ...
      ride = await Ride.findOne({ blabla_ride_id: blabla_ride_id });
    }
    if (!ride) throw new Error("No ride found");
    let booking;
    if (user_phone) {
